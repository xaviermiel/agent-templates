...
3. Rebuild and restart the server — **both steps are required** for changes to go live:
   ```bash
   npm run build && pkill -f 'node dist/server/entry.mjs' || true
   ```
...
```bash
node -e "const db = require('better-sqlite3')('data/database.db'); console.log(db.prepare('SELECT COUNT(*) as count FROM waitlist').get());"
```
...
You can run scheduled tasks for the user — daily reports, data summaries, signup digests, whatever fits their project. The `tasks` array in the agent's `manifest.json` (two levels above this folder, alongside `workspace/`) is empty by default because every project is different.
